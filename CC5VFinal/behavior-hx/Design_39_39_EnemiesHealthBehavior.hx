...
class Design_39_39_EnemiesHealthBehavior extends ActorScript
{          	
...
{
if(wrapper.enabled && sameAsAny(getActorGroup(10),event.otherActor.getType(),event.otherActor.getGroup()))
{
        if(Engine.engine.getGameAttribute("GameIsOn"))
{
            recycleActor(actor.getLastCollidedActor());
            actor.setFilter([createNegativeFilter()]);
            _CurrentHealth = asNumber((_CurrentHealth - 1));
propertyChanged("_CurrentHealth", _CurrentHealth);
